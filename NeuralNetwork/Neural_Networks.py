...
def NN(X,Y,label):
    ...
    #no_H=int(no_samples/(2*(no_I+no_O)))
    ...
    while(epoch > 0):
        ...
        dH_out_dH_in=derivative_sigmoid(H_in)
        dH_in_dW_H=X
    ...
    print("loss-",error_cost)
    plt.plot(epoches, error_cost ,'b-')
...
def main():
    ...
    W_H,B_H,W_O,B_O,output=NN(train_X,train_Y,label)
    ...
    output=forward(valid_X,W_H,B_H,W_O,B_O)
    correct=0
    ...
    file="../Apparel/apparel-test.csv"
    test_df=pd.read_csv(file)
    test_X=to_array(test_df)
    output_file="2018201054_prediction.csv"
    f = open(output_file,"w")
    
    output=forward(test_X,W_H,B_H,W_O,B_O)
    for i in range(len(test_X)):
        prediction=np.argmin(output[i])
        writer=csv.writer(f)
        writer.writerows(str(prediction))
    f.close()
